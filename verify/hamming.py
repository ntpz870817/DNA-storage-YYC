...
def get_hamming_length(matrix_length):
...
def add_code(binary_list):

    log.output(log.NORMAL, str(__name__), str(sys._getframe().f_code.co_name),
               "Add hamming code to a line list.")

    new_binary_list = []
    total_length = len(binary_list) + get_hamming_length(len(binary_list))
    ...
    for index in range(total_length):
        ...
        if int(math.pow(2, verify_position)) == index:
            ...
            while calculated_position < total_length:
                new_binary_list[index] = xor(new_binary_list[index], new_binary_list[calculated_position])
                calculated_count -= 1
    ...
    for index in range(len(binary_list)):
        ...
        if int(math.pow(2, verify_position)) == index:
            ...
            while calculated_position < len(binary_list):
                result = xor(result, binary_list[calculated_position])
                calculated_count -= 1
                calculated_position += 1
                if calculated_count == 0:
                    calculated_count = index + 1
